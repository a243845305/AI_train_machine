...
def geometryDistance(h, w, device=torch.device('cpu')):
    x_list = np.arange(0, w, dtype=np.float32)
    y_list = np.arange(0, h, dtype=np.float32)
    ...
    dist = (grid.view(1, w * h, 2) - grid.view(w * h, 1, 2))
    dist = dist*dist
    dist = dist.sum(dim=-1)
    dist /= dist.max()
    return dist
...
def sinkhornDistance(hm1, hm2, dist=None):
    assert hm1.shape == hm2.shape

    if dist is None:
        dist = geometryDistance(hm1.shape[0], hm1.shape[1])

    hm1 = hm1.contiguous().view(1, -1)
    hm2 = hm2.contiguous().view(1, -1)
    ...
    w_dist = SinkhornOT.apply(hm1, hm2, dist, 1e-3, 200)
...
if __name__ == '__main__':
    ...
    tic = time.time()
    dist = geometryDistance(96, 160)
    np_dist = dist.cpu().numpy()
